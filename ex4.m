...
Wp=(2*fcut)/fe;% borne inférieur de la bande passante
Ws=2*(fcut+largeur)/fe;%  borne supérieur de la bande passante
...
stem (y) ;
xlim([0,150]);
my_title ('Reponse impulsionnelle') ;
...
plot (f,20*abs(H),'b');
xlim([0,1]);
my_title ('Fonction de transfert', 13) ;

% Signal constitué par la somme de deux sinusoides
input ('Figure suivante ? ') ;
Te = 1/fe;%Periode d'échantionnalle
...
plot(abs(fFX));
xlim([0,1100]);
my_title ('Spectre du signal filtre') ;
